#!/bin/bash
# ==========================================================
# MultiChain Development Node Installation for Procuchain
# Creates a separate blockchain for local development
# to keep production data isolated from development
# ==========================================================
...
if [[ ! -f "${CHAIN_DIR}/params.dat" ]]; then
    ...
    multichain-util create "${CHAIN_NAME}" \
        -default-network-port=7449 \
        -default-rpc-port=7450
else
...
[[ -z "${RPC_PORT}" ]] && RPC_PORT=7450
[[ -z "${P2P_PORT}" ]] && P2P_PORT=7449

echo ">>> Starting '${CHAIN_NAME}' development node"
...
if ! multichain-cli "${CHAIN_NAME}" getinfo >/dev/null 2>&1; then
    echo "Failed to communicate with the development node." >&2
    exit 1
fi
...
if [[ ${needs_restart} -eq 1 ]]; then
    echo ">>> Restarting development node to apply RPC settings"
...
RPC_HOST=$(curl -fsSL ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}' || echo "127.0.0.1")

echo ""
echo "----------------------------------------------------"
echo " 🔧 MultiChain '${CHAIN_NAME}' DEVELOPMENT node is running"
echo " P2P Port : ${P2P_PORT}"
echo " RPC Port : ${RPC_PORT}"
echo " RPC User : ${RPC_USER}"
echo "----------------------------------------------------"
echo " Connect other dev nodes with:"
echo "   multichaind ${CHAIN_NAME}@${RPC_HOST}:${P2P_PORT}"
echo "----------------------------------------------------"

echo ""
echo "===================================================="
echo " 🔧 Laravel .env Configuration (LOCAL DEVELOPMENT)"
echo "===================================================="
...
echo ""
echo "⚠️  IMPORTANT: This is a DEVELOPMENT blockchain"
echo "   • Use this configuration in your LOCAL .env only"
echo "   • Do NOT use these credentials for production"
echo "   • Production uses: procuchain @ 159.65.12.99:6486"
echo ""
echo "Next steps:"
echo "  1. Copy the values above into your LOCAL .env file"
echo "  2. Test the RPC connection: php artisan multichain:setup --check"
echo "  3. Finish setup           : php artisan multichain:setup"
echo "  4. Run database seeders to populate dev blockchain"
echo ""
echo "✅ Development blockchain is now isolated from production!"
