...
class SharedLedgerService
{
        ...
        try {
            ...
            foreach (self::LEDGER_STREAMS as $stream) {
                ...
                foreach ($items as $item) {
                    ...
                    try {
                        $entries[] = $this->entryFromStreamItem($stream, $item);
                    } catch (Exception $e) {
                        report($e);
                    }
                }
    ...
    private function entryFromStreamItem(string $stream, array $item): array
    {
        $data = $item['data']['json'] ?? [];
        $txid = $item['txid'] ?? '';
        $key = $item['keys'][0] ?? '';
        ...
        if ($timestamp !== null) {
            $formatted_timestamp = Carbon::createFromTimestamp((int) $timestamp)->toIso8601String();
        } else {
        ...
        if ($formatted_timestamp === null && isset($item['confirm'])) {
            $formatted_timestamp = Carbon::now()->toIso8601String();
        }
