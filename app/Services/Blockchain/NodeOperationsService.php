 ...
 */
 ...
 'fi',
 '',
 '# Step 6: Unsubscribe from all streams to prevent auto-resync',
 '# MultiChain auto-subscribes to streams on startup; we must unsubscribe',
 '# so the node stays empty until the user manually resyncs.',
 'STREAMS=$($CLI liststreams -ci 2>/dev/null | grep -oP \'"name"\\s*:\\s*"\\K[^"]+\' || true)',
 'UNSUB_COUNT=0',
 'for S in $STREAMS; do',
 ' if [ "$S" = "root" ]; then continue; fi',
 ' $CLI unsubscribe "$S" 2>/dev/null && UNSUB_COUNT=$((UNSUB_COUNT + 1)) || true',
 'done',
 'echo "Unsubscribed from ${UNSUB_COUNT} streams"',
 '',
 '# Step 7: Verify 0 items in subscribed streams',
 'TOTAL_ITEMS=0',
 'for S in $STREAMS; do',
 ' if [ "$S" = "root" ]; then continue; fi',
 ' # Only count items from still-subscribed streams (should be none)',
 ' ITEMS=$($CLI getstreaminfo "$S" 2>/dev/null | grep -oP \'"items"\\s*:\\s*\\K[0-9]+\' || echo "0")',
 ' TOTAL_ITEMS=$((TOTAL_ITEMS + ITEMS))',
 'done',
 'echo "PURGE_SUCCESS: Daemon restarted and unsubscribed, total_items=$TOTAL_ITEMS, unsubscribed=$UNSUB_COUNT"',
            ]);
