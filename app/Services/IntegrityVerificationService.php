...
class IntegrityVerificationService
{
    ...
    private function verifyAllTables(): void
    {
        ...
        Procurement::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurements', Stream::METADATA);
            }
        });
        ...
        ProcurementStage::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurement_stages', Stream::STATUS);
            }
        });
        ...
        ProcurementDocument::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurement_documents', Stream::DOCUMENTS);
            }
        });
        ...
        ProcurementEvent::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurement_events', Stream::EVENTS);
            }
        });

        ProcurementCorrection::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurement_corrections', Stream::CORRECTIONS);
            }
        });

        ProcurementArchive::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurement_archives', Stream::ARCHIVE);
            }
        });

        ProcurementMetadataCorrection::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'procurement_metadata_corrections', Stream::PROCUREMENTS_CORRECTIONS);
            }
        });

        File::chunk(100, function ($records) {
            foreach ($records as $record) {
                $this->verifyRecord($record, 'Files', Stream::FILE_METADATA);
            }
        });
    }
     ...
     */
    private function verifyRecord(Model $record, string $tableName, Stream $stream): void
    {
        $this->verifiedCount++;
    ...
    private function checkUnauthorizedPublisher(Model $record, string $tableName, string $prNumber, string $stream): bool
    {
        try {
            ...
            $txData = $this->BlockchainRpcClient->getrawtransaction($txid, 1);
        ...
        try {
            ...
            foreach ($fakeRecords as $record) {
                ...
                if ($record->txid) {
                    ...
                    if (! $chainData) {
                        try {
                            $txData = $this->BlockchainRpcClient->getrawtransaction($record->txid, 1);
                            if (is_array($txData)) {
    ...
    private function contentViolationIsResolved(IntegrityViolationLog $violation, ?string $modelClass): bool
    {
        ...
        if (! $this->recordReferencesLatestChainRevision($record, $tableName, $prNumber, Stream::from($violation->stream))) {
            return false;
        }
    ...
    {
        $pending = IntegrityViolationLog::forRun($this->runId)
            ->where('recovery_status', 'pending')
            ->get();
    ...
    private function fetchChainData(string $stream, string $prNumber, ?string $txid): ?array
    {
        try {
            ...
            $items = $this->BlockchainRpcClient->liststreamkeyitems($stream, $prNumber);
    ...
    private function notifyBreach(string $type, string $prNumber, string $message, ?array $fieldDiffs = null, ?string $stream = null, ?string $txid = null): void
    {
        try {
            ...
            // The daily digest job will pick up violations from IntegrityViolationLog.
