...
import type { UseFormData, CategoryOption, ProcurementModeOption, NegotiatedProcurementTypeOption } from '@/hooks/use-procurement-initiation';
            ...
            >
              {FUNDING_SOURCES.map((source) => (
                <div key={source.value} className="flex items-center gap-2">
                  <RadioGroupItem value={source.value} id={`funding-${source.value}`} className={hasError('funding_source') ? 'border-destructive' : ''} />
                  ...
                  {negotiatedProcurementTypes.map((type) => (
                    <div
                      ...
                      className={`flex items-center gap-3 rounded-lg border p-2.5 transition-colors ${
                        data.negotiated_procurement_type === type.value ? 'border-primary bg-primary/5' : 'border-input hover:bg-muted/50'
