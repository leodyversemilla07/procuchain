...
export const KanbanBoard = ({
    ...
    const phases = [...new Set(procurements.map(proc => proc.stage))] as Stage[];
    const procurementsByPhase: Record<string, ProcurementListItem[]> = {};
    phases.forEach(phase => {
        procurementsByPhase[phase] = procurements.filter(proc => proc.stage === phase);
    });

    return (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 overflow-x-auto">
            {phases.map(phase => (
                <Card key={phase} className="border-sidebar-border/70 dark:border-sidebar-border rounded-lg p-3 min-w-[240px]">
                    <div className="flex items-center justify-between mb-3">
                        <h3 className="font-medium text-sm flex items-center gap-2 dark:text-gray-200">
                            <Badge variant="outline" className={`${getStageBadgeStyle(phase)} whitespace-nowrap`}>
                                {phase}
                            </Badge>
