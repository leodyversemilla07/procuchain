...
export function useLoginLogFilters({ recentLogins, suspiciousActivities, flash }: UseLoginLogFiltersOptions) {
  ...
  useEffect(() => {
    ...
    if (searchTerm !== debouncedSearchTerm) { setIsLoading(true); }
  ...
  const combinedFilteredAndSortedLogs: CombinedLog[] = useMemo(() => {
    ...
    logs = logs.filter((log) => {
      ...
      const matchesDateRange =
        ...
        (() => { const d = new Date(log.login_at); return d >= dateRange.from! && d <= dateRange.to!; })();
  ...
  useEffect(() => { setCombinedPage(1); }, [debouncedSearchTerm, selectedRole, selectedStatus, selectedDeviceType, selectedBrowser, dateRange, selectedCategory]);

  // Unique filter options
  const getUniqueRoles = useMemo(() => {
    const roles = new Set<string>();
    [...recentLogins, ...(suspiciousActivities || [])].forEach((l) => { if (l.user?.primary_role) roles.add(l.user.primary_role); });
    return Array.from(roles).sort();
  }, [recentLogins, suspiciousActivities]);

  const getUniqueBrowsers = useMemo(() => {
    const browsers = new Set<string>();
    [...recentLogins, ...(suspiciousActivities || [])].forEach((l) => { if (l.browser) browsers.add(l.browser); });
    return Array.from(browsers).sort();
  }, [recentLogins, suspiciousActivities]);

  const getUniqueDeviceTypes = useMemo(() => {
    const types = new Set<string>();
    [...recentLogins, ...(suspiciousActivities || [])].forEach((l) => { if (l.device_type) types.add(l.device_type); });
  ...
  const hasActiveFilters = useMemo(() => (
    ...
    selectedDeviceType !== 'all' || selectedBrowser !== 'all' ||
  ...
  const exportToCSV = useCallback(() => {
    setIsExporting(true);
    try {
      const logs = selectedLogs.size > 0
        ? combinedFilteredAndSortedLogs.filter((l) => selectedLogs.has(l.id))
      ...
      const rows = logs.map((log) => [
        formatDateTime(log.login_at), log.user?.name || 'Unknown', log.user?.email || 'Unknown',
        log.user?.primary_role || '-', log.user?.two_factor_enabled ? 'Enabled' : 'Disabled',
        log.successful ? 'Success' : 'Failed', log.ip_address, log.location || '-',
        log.device_type || 'Unknown', log.browser || 'Unknown', log.platform || '-',
  ...
  const handleRefresh = useCallback(() => {
    ...
    router.reload({ only: ['recentLogins', 'statistics', 'suspiciousActivities'], onFinish: () => { setIsRefreshing(false); toast.success('Data refreshed'); } });
  }, []);
  ...
  const clearAllFilters = useCallback(() => {
    setSearchTerm(''); setSelectedRole('all'); setSelectedStatus('all');
    setSelectedDeviceType('all'); setSelectedBrowser('all');
    setDateRange(undefined); setSelectedCategory('all');
  }, []);
  ...
  const setDateRangePreset = useCallback((preset: string) => {
    ...
    switch (preset) {
      case 'today': setDateRange({ from: startOfToday, to: endOfToday }); break;
      case 'last7days': { const d = new Date(today); d.setDate(today.getDate() - 7); setDateRange({ from: d, to: today }); break; }
      case 'last30days': { const d = new Date(today); d.setDate(today.getDate() - 30); setDateRange({ from: d, to: today }); break; }
      case 'thisMonth': setDateRange({ from: new Date(today.getFullYear(), today.getMonth(), 1), to: new Date(today.getFullYear(), today.getMonth() + 1, 0) }); break;
      default: setDateRange(undefined);
    }
  }, []);

  const handleViewDetails = useCallback((log: LoginLog, category: 'recent' | 'suspicious') => {
    setSelectedLog(log); setSelectedLogCategory(category); setIsDetailsDialogOpen(true);
  }, []);

  const handleBlockIpClick = useCallback((ip: string) => { setIpToBlock(ip); setIsBlockDialogOpen(true); }, []);

  const handleBlockIpConfirm = useCallback(async (reason: string, duration: 'temporary' | 'permanent') => {
    if (!ipToBlock) return;
    setIsBlocking(true);
    router.post(loginLogs.blockIp.url(), { ip_address: ipToBlock, reason, duration }, {
      preserveScroll: true,
      onSuccess: () => { setIsBlockDialogOpen(false); setIpToBlock(null); },
  ...
  return {
    ...
    selectedDeviceType, setSelectedDeviceType, selectedBrowser, setSelectedBrowser,
    dateRange, setDateRange, showAdvancedFilters, setShowAdvancedFilters,
    selectedCategory, setSelectedCategory,
    isLoading, isRefreshing, autoRefresh, setAutoRefresh,
    selectedLogs, isExporting,
    combinedPage, setCombinedPage, pageSize, setPageSize,
    // Computed
    combinedFilteredAndSortedLogs, paginatedCombinedLogs, totalCombinedPages,
    hasActiveFilters, getUniqueRoles, getUniqueBrowsers, getUniqueDeviceTypes,
    // Dialog
    selectedLog, selectedLogCategory, isDetailsDialogOpen, setIsDetailsDialogOpen,
    ipToBlock, isBlockDialogOpen, setIsBlockDialogOpen, isBlocking,
    // Actions
    exportToCSV, handleRefresh, toggleSelectAll, toggleLogSelection,
    clearAllFilters, setDateRangePreset, handleViewDetails, handleBlockIpClick, handleBlockIpConfirm,
  };
}
...
export function highlightSearchTerm(text: string, term: string): React.ReactNode {
  ...
  return parts.map((part, i) =>
    regex.test(part)
      ? <span key={i} className="rounded bg-yellow-200 px-0.5 dark:bg-yellow-800">{part}</span>
      : part
  );
}

export function formatDateTime(dateString: string) {
  try { const d = new Date(dateString); return isNaN(d.getTime()) ? 'Invalid Date' : d.toLocaleString(); }
