...
const formSchema = z.object({
  ntpFile: z.instanceof(File, {
    message: "Notice to Proceed file is required"
  }),
  issuanceDate: z.date({
...
export default function NTPUpload({ procurement, errors = {} }: NTPUploadProps) {
  const [isDraggingNtp, setIsDraggingNtp] = useState(false);
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    formData.append('ntp_file', values.ntpFile);
      ...
      <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-gradient-to-b from-background to-muted/20">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-primary">
            ...
            <h1 className="text-2xl font-bold">Notice to Proceed (NTP)</h1>
          </div>
          <p className="text-muted-foreground max-w-3xl">
            Upload the Notice to Proceed document for procurement
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md lg:col-span-2">
                <CardHeader className="pb-4 space-y-1">
                  ...
                  <CardDescription>
                    Please upload the Notice to Proceed in PDF format
                  </CardDescription>
                </CardHeader>
                ...
                <CardContent className="space-y-8">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl>
                          <div
                            className={`border-2 border-dashed rounded-lg p-6 transition-all duration-200 min-h-[220px] flex flex-col justify-center ${isDraggingNtp
                              ? 'border-primary bg-primary/5 scale-[1.01] shadow-md'
                              : field.value
                                ? 'border-green-500/50 bg-green-50 dark:bg-green-900/20'
                                : form.formState.errors.ntpFile
                                  ? 'border-destructive/50 bg-destructive/5 dark:bg-destructive/10'
                                  : 'border-muted-foreground/25 hover:border-primary/50 hover:bg-muted/50'
                              } cursor-pointer group`}
                            onDragEnter={(e) => handleDragEvents(e, setIsDraggingNtp)}
                            onDragLeave={(e) => handleDragEvents(e, setIsDraggingNtp, false)}
                            onDragOver={(e) => handleDragEvents(e, setIsDraggingNtp)}
                            onDrop={(e) => handleFileDrop(e, "ntpFile", setIsDraggingNtp)}
                            onClick={() => document.getElementById('ntp-file-input')?.click()}
                          >
                            {!field.value ? (
                              <div className="flex flex-col items-center justify-center text-center">
                                ...
                                <p className="font-medium text-muted-foreground mb-2 group-hover:text-foreground transition-colors">
                                  Drag and drop your NTP file here
                                </p>
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    document.getElementById('ntp-file-input')?.click();
                                  }}
                                >
                                  Browse Files
                                </Button>
                                <input
                                  ...
                                  className="hidden"
                                  onChange={(e) => handleFileChange(e, "ntpFile")}
                                />
                              </div>
                            ) : (
                  ...
                  <div className="bg-cyan-50 dark:bg-cyan-900/20 p-4 rounded-md border border-cyan-200 dark:border-cyan-800">
                    <div className="flex items-start">
                      <PlayCircle className="h-5 w-5 text-cyan-600 dark:text-cyan-500 mt-0.5 mr-3 flex-shrink-0" />
                      <p className="text-sm text-cyan-700 dark:text-cyan-500">
                        The Notice to Proceed (NTP) is the formal instruction to the supplier to begin work on the project.
                        This document should be published to PhilGEPS website for compliance before or after the upload.
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md h-fit">
                ...
                <CardFooter className="pt-4 border-t flex flex-col gap-3">
                  <Button
                    ...
                    className="w-full flex items-center gap-2 h-11 bg-cyan-600 hover:bg-cyan-700"
                  >
                    {isSubmitting ? (
