...
const formSchema = z.object({
  contractFile: z.instanceof(File, {
    message: "Contract file is required"
  }),
  poFile: z.instanceof(File, {
    message: "Purchase Order file is required"
  }),
  signingDate: z.date({
    required_error: "Signing date is required",
  }),
});
...
export default function ContractPOUpload({ procurement, errors = {} }: ContractPOUploadProps) {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    formData.append('contract_file', values.contractFile);
    formData.append('po_file', values.poFile);
    formData.append('signing_date', format(values.signingDate, 'yyyy-MM-dd'));

    router.post('/bac-secretariat/upload-contract-po-documents', formData, {
      onSuccess: () => {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Upload Contract and PO" />
      ...
      <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-gradient-to-b from-background to-muted/20">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-primary">
            ...
            <h1 className="text-2xl font-bold">Contract and Purchase Order Documents</h1>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <CardContent className="space-y-8">
                          ...
                          <div
                            className={`border-2 border-dashed rounded-lg p-6 transition-all duration-200 min-h-[220px] flex flex-col justify-center ${isDraggingContract
                              ...
                              : field.value
                                ...
                                : form.formState.errors.contractFile
                                  ? 'border-destructive/50 bg-destructive/5 dark:bg-destructive/10'
                                  : 'border-muted-foreground/25 hover:border-primary/50 hover:bg-muted/50'
                              } cursor-pointer group`}
                            onDragEnter={(e) => handleDragEvents(e, setIsDraggingContract)}
                            onDragLeave={(e) => handleDragEvents(e, setIsDraggingContract, false)}
                            onDragOver={(e) => handleDragEvents(e, setIsDraggingContract)}
                            onDrop={(e) => handleFileDrop(e, "contractFile", setIsDraggingContract)}
                        ...
                        <FormControl>
                          <div
                            className={`border-2 border-dashed rounded-lg p-6 transition-all duration-200 min-h-[220px] flex flex-col justify-center ${isDraggingPO
                              ...
                              : field.value
                                ? 'border-green-500/50 bg-green-50 dark:bg-green-900/20'
                                : form.formState.errors.poFile
                                  ? 'border-destructive/50 bg-destructive/5 dark:bg-destructive/10'
                                  : 'border-muted-foreground/25 hover:border-primary/50 hover:bg-muted/50'
                              } cursor-pointer group`}
                            onDragEnter={(e) => handleDragEvents(e, setIsDraggingPO)}
                            onDragLeave={(e) => handleDragEvents(e, setIsDraggingPO, false)}
                            onDragOver={(e) => handleDragEvents(e, setIsDraggingPO)}
                            onDrop={(e) => handleFileDrop(e, "poFile", setIsDraggingPO)}
                          ...
                          >
                            {!field.value ? (
                              <div className="flex flex-col items-center justify-center text-center">
                                ...
                                <p className="font-medium text-muted-foreground mb-2 group-hover:text-foreground transition-colors">
                                  Drag and drop your purchase order file here
                                </p>
                                <p className="text-sm text-muted-foreground/70 mb-5">
                                  Only PDF files are supported
                                </p>
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-md border border-yellow-200 dark:border-yellow-800">
                    <div className="flex items-start">
                      <Briefcase className="h-5 w-5 text-yellow-600 dark:text-yellow-500 mt-0.5 mr-3 flex-shrink-0" />
                      <p className="text-sm text-yellow-700 dark:text-yellow-500">
                        Both the contract and purchase order documents must be finalized and signed before upload.
                        This will complete the Contract and PO phase.
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md h-fit">
                <CardHeader className="pb-4 space-y-1">
                  ...
                  <CardDescription>
                    Provide information about the Contract and PO
                  </CardDescription>
                </CardHeader>
