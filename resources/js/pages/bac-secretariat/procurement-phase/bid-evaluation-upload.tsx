...
import { CalendarIcon, FileText, Upload, AlertCircle, BarChart4, X, FileUp, Users } from 'lucide-react';
import {
  Card,
  CardContent,
  CardDescription,
...
const formSchema = z.object({
  summaryFile: z.instanceof(File, {
    message: "Evaluation summary file is required"
  }),
  ...
  evaluatorNames: z.string().min(1, "Evaluator names are required"),
});
...
export default function BidEvaluationUpload({ procurement, errors = {} }: BidEvaluationUploadProps) {
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Upload Bid Evaluation Documents - ${procurement.id}`, href: '#' },
  ];
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    ...
    formData.append('summary_file', data.summaryFile);
    formData.append('abstract_file', data.abstractFile);
    formData.append('evaluation_date', format(data.evaluationDate, 'yyyy-MM-dd'));
    formData.append('evaluator_names', data.evaluatorNames);

    router.post('/bac-secretariat/upload-bid-evaluation-documents', formData, {
      forceFormData: true,
      onSuccess: () => {
  ...
  const handleSummaryDragEnter = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleSummaryDragOver = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!isDraggingSummary) setIsDraggingSummary(true);
  };
  ...
  const handleSummaryDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (file.type === 'application/pdf') {
        form.setValue("summaryFile", file, { shouldValidate: true });
      } else {
        form.setError("summaryFile", {
          type: "manual",
  ...
  const handleAbstractDragEnter = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDraggingAbstract(true);
  };

  const handleAbstractDragLeave = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDraggingAbstract(false);
  };

  const handleAbstractDragOver = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!isDraggingAbstract) setIsDraggingAbstract(true);
  };

  const handleAbstractDrop = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDraggingAbstract(false);
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      const file = e.dataTransfer.files[0];
      if (file.type === 'application/pdf') {
        form.setValue("abstractFile", file, { shouldValidate: true });
      } else {
        form.setError("abstractFile", {
          type: "manual",
          message: "Only PDF files are allowed"
        });
      }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: "summaryFile" | "abstractFile") => {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Upload Bid Evaluation Documents" />
      ...
      <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-gradient-to-b from-background to-muted/20">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-primary">
            ...
            <h1 className="text-2xl font-bold">Bid Evaluation Documents</h1>
          </div>
          <p className="text-muted-foreground max-w-3xl">
            Upload evaluation summary and abstract for procurement
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl>
                          <div
                            className={`border-2 border-dashed rounded-lg p-6 transition-all duration-200 min-h-[220px] flex flex-col justify-center ${isDraggingSummary
                              ? 'border-primary bg-primary/5 scale-[1.01] shadow-md'
                              : field.value
                                ? 'border-green-500/50 bg-green-50 dark:bg-green-900/20'
                                : form.formState.errors.summaryFile
                            ...
                            {!field.value ? (
                              <div className="flex flex-col items-center justify-center text-center">
                                ...
                                <p className="font-medium text-muted-foreground mb-2 group-hover:text-foreground transition-colors">
                                  Drag and drop your evaluation summary file here
                                </p>
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    document.getElementById('summary-file-input')?.click();
                                  }}
                                >
                                  Browse Files
                                </Button>
                        ...
                        <FormControl>
                          <div
                            className={`border-2 border-dashed rounded-lg p-6 transition-all duration-200 min-h-[220px] flex flex-col justify-center ${isDraggingAbstract
                              ...
                              : field.value
                                ? 'border-green-500/50 bg-green-50 dark:bg-green-900/20'
                                : form.formState.errors.abstractFile
                                  ? 'border-destructive/50 bg-destructive/5 dark:bg-destructive/10'
                                  : 'border-muted-foreground/25 hover:border-primary/50 hover:bg-muted/50'
                              } cursor-pointer group`}
                            onDragEnter={handleAbstractDragEnter}
                            onDragLeave={handleAbstractDragLeave}
                            onDragOver={handleAbstractDragOver}
                            onDrop={handleAbstractDrop}
                          ...
                          >
                            {!field.value ? (
                              <div className="flex flex-col items-center justify-center text-center">
                                ...
                                <p className="font-medium text-muted-foreground mb-2 group-hover:text-foreground transition-colors">
                                  Drag and drop your bid abstract file here
                                </p>
                                <p className="text-sm text-muted-foreground/70 mb-5">
                                  Only PDF files are supported
                                </p>
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    document.getElementById('abstract-file-input')?.click();
                                  }}
                                >
                                  Browse Files
                                </Button>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Enter evaluator names separated by commas (e.g., Alice, Bob)"
                            rows={5}
