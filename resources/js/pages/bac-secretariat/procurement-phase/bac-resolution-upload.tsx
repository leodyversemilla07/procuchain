...
const formSchema = z.object({
  bacResolutionFile: z.instanceof(File, {
    message: "BAC Resolution file is required"
  }),
  issuanceDate: z.date({
...
export default function BacResolutionUpload({ procurement, errors = {} }: BacResolutionUploadProps) {
  const [isDraggingResolution, setIsDraggingResolution] = useState(false);
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    formData.append('bac_resolution_file', values.bacResolutionFile);
    ...
    router.post('/bac-secretariat/upload-bac-resolution-document', formData, {
      onSuccess: () => {
      ...
      <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-gradient-to-b from-background to-muted/20">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-primary">
            ...
            <h1 className="text-2xl font-bold">BAC Resolution Document</h1>
          </div>
          <p className="text-muted-foreground max-w-3xl">
            Upload the BAC Resolution document for procurement
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md lg:col-span-2">
                <CardHeader className="pb-4 space-y-1">
                  ...
                  <CardDescription>
                    Please upload the BAC Resolution document in PDF format
                  </CardDescription>
                </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl>
                          <div
                            className={`border-2 border-dashed rounded-lg p-6 transition-all duration-200 min-h-[220px] flex flex-col justify-center ${isDraggingResolution
                              ? 'border-primary bg-primary/5 scale-[1.01] shadow-md'
                              : field.value
                                ? 'border-green-500/50 bg-green-50 dark:bg-green-900/20'
                                : form.formState.errors.bacResolutionFile
                                  ? 'border-destructive/50 bg-destructive/5 dark:bg-destructive/10'
                                  : 'border-muted-foreground/25 hover:border-primary/50 hover:bg-muted/50'
                              } cursor-pointer group`}
                            onDragEnter={(e) => handleDragEvents(e, setIsDraggingResolution)}
                            onDragLeave={(e) => handleDragEvents(e, setIsDraggingResolution, false)}
                            onDragOver={(e) => handleDragEvents(e, setIsDraggingResolution)}
                            onDrop={(e) => handleFileDrop(e, "bacResolutionFile", setIsDraggingResolution)}
                            onClick={() => document.getElementById('bac-resolution-file-input')?.click()}
                          >
                            {!field.value ? (
                              <div className="flex flex-col items-center justify-center text-center">
                                ...
                                <p className="font-medium text-muted-foreground mb-2 group-hover:text-foreground transition-colors">
                                  Drag and drop your BAC Resolution file here
                                </p>
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    document.getElementById('bac-resolution-file-input')?.click();
                                  }}
                                >
                                  Browse Files
                                </Button>
                                <input
                                  id="bac-resolution-file-input"
                                  type="file"
                                  accept="application/pdf"
                                  className="hidden"
                                  onChange={(e) => handleFileChange(e, "bacResolutionFile")}
                                />
                              </div>
                            ) : (
                              <div className="flex items-center justify-between">
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    form.resetField("bacResolutionFile");
                                  }}
                                >
                                  <X className="h-4 w-4" />
                                </Button>
                  ...
                  <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-md border border-amber-200 dark:border-amber-800">
                    <div className="flex items-start">
                      ...
                      <p className="text-sm text-amber-700 dark:text-amber-400">
                        Uploading this document will finalize the BAC Resolution phase and move the procurement to the Notice of Award phase.
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md h-fit">
                <CardHeader className="pb-4 space-y-1">
                  ...
                  <CardDescription>
                    Provide details about the BAC resolution
                  </CardDescription>
                </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Enter names and positions of BAC resolution signatories"
                            rows={5}
