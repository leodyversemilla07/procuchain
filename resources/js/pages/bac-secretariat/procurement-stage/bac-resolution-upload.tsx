...
import { AlertCircle, ArrowRight, CheckCircle2, FileCheck2 } from 'lucide-react';
...
export default function BacResolutionUpload({ procurement, workflowInfo, documentGuide, uploadedDocuments = [] }: BacResolutionUploadProps) {
    ...
    const [confirmDialog, setConfirmDialog] = useState<{ open: boolean; documentValue: string; documentName: string }>({
        open: false,
        documentValue: '',
        documentName: '',
    });
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload BAC Resolution - ${procurement?.pr_number || 'Unknown'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        setIsMarkingComplete(true);
        router.post(
            markStageComplete({ pr_number: procurement.pr_number, stage: 'bac_resolution' }).url,
            {},
            {
                onSuccess: (page) => {
    ...
    const handleConfirmUpload = useCallback(() => {
        ...
        router.post(
            uploadSingleDocument({ pr_number: procurement.pr_number, stage: 'bac_resolution' }).url,
            {
                document_file: file,
                    ...
                    <CardContent className="flex flex-col gap-2 p-4 sm:p-6">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div className="text-primary flex items-center gap-2">
                                ...
                                <h1 className="text-xl font-bold sm:text-2xl">BAC Resolution</h1>
                            </div>
