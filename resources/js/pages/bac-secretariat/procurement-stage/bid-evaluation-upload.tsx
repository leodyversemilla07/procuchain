...
export default function BidEvaluationUpload({ procurement, documentGuide, uploadedDocuments = [] }: BidEvaluationUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Bid Evaluation - ${procurement?.pr_number || 'Unknown'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        setIsMarkingComplete(true);
        router.post(
            markStageComplete({ pr_number: procurement.pr_number, stage: 'bid_evaluation' }).url,
            {},
            {
                onSuccess: (page) => {
    ...
    const isStageCompleted = procurement.stage_value === 'post_qualification' || 
                              (procurement.stage_value === 'bid_evaluation' && procurement.status === 'bids_evaluated');
    ...
    const handleConfirmUpload = useCallback(() => {
        ...
        router.post(
            uploadSingleDocument({ pr_number: procurement.pr_number, stage: 'bid_evaluation' }).url,
            {
                document_file: file,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Upload Bid Evaluation" />
            ...
            <div className="from-background to-muted/20 flex h-full flex-1 flex-col gap-4 rounded-xl bg-linear-to-b p-4 sm:gap-6 sm:p-6">
                <div className="flex flex-col gap-2">
                    <div className="text-primary flex items-center gap-2">
                        ...
                        <h1 className="text-xl font-bold sm:text-2xl">Bid Evaluation</h1>
                    </div>
                    <p className="text-muted-foreground text-sm sm:text-base">
                        Upload the Bid Evaluation for procurement
                            ...
                            <Card className="border-sidebar-border/70 dark:border-sidebar-border h-fit shadow-md">
                                <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                    <CardTitle className="flex items-center gap-2 text-lg font-semibold sm:text-xl">
                                        <FileCheck className="text-primary h-4 w-4 sm:h-5 sm:w-5" />
                                        Upload Progress
                                    </CardTitle>
                        ...
                        <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md lg:col-span-2">
                            <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                ...
                                <CardDescription className="text-sm">
                                    Upload required and optional documents for Bid Evaluation. Files will be permanently saved.
                                </CardDescription>
                            </CardHeader>
