...
export default function BidEvaluationUpload({ procurement, documentGuide, uploadedDocuments = [] }: BidEvaluationUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Bid Evaluation - ${procurement?.id || 'Unknown ID'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        router.post(
            markStageComplete({ pr_number: procurement.id, stage: 'bid_evaluation' }).url,
            {},
            {
                onSuccess: () => {
