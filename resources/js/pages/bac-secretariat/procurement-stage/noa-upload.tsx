...
export default function NoaUpload({ procurement, documentGuide, uploadedDocuments = [] }: NoaUploadProps) {
    // Progressive upload hook
    const { isUploading, handleDocumentUpload } = useProgressiveUpload({
        procurementId: procurement?.id || '',
        stage: 'notice_of_award',
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Notice of Award - ${procurement?.id || 'Unknown ID'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        router.post(
            markStageComplete({ pr_number: procurement.id, stage: 'notice_of_award' }).url,
            {},
            {
                onSuccess: () => {
