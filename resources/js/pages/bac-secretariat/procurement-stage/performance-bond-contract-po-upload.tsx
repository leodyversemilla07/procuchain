...
import { markStageComplete } from '@/actions/App/Http/Controllers/Procurement/PostProcurementController';
...
export default function PerformanceBondContractPoUpload({ procurement, documentGuide, uploadedDocuments = [] }: PerformanceBondContractPoUploadProps) {
    ...
    const { isUploading, handleDocumentUpload } = useProgressiveUpload({
        ...
        stage: 'performance_bond_contract_and_po',
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Performance Bond - ${procurement?.id || 'Unknown ID'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        router.post(
            markStageComplete({ pr_number: procurement.id, stage: 'performance_bond_contract_and_po' }).url,
            {},
            {
                onSuccess: () => {
