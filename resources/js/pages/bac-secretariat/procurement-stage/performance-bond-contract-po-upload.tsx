...
export default function PerformanceBondContractPoUpload({ procurement, documentGuide, uploadedDocuments = [] }: PerformanceBondContractPoUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Performance Bond - ${procurement?.pr_number || 'Unknown'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        router.post(
            markStageComplete({ pr_number: procurement.pr_number, stage: 'performance_bond_contract_and_po' }).url,
            {},
            {
                onSuccess: (page) => {
    ...
    const handleConfirmUpload = useCallback(() => {
        ...
        router.post(
            uploadSingleDocument({ pr_number: procurement.pr_number, stage: 'performance_bond_contract_and_po' }).url,
            {
                document_file: file,
                            ...
                            <Card className="border-sidebar-border/70 dark:border-sidebar-border h-fit shadow-md">
                                <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                    <CardTitle className="flex items-center gap-2 text-lg font-semibold sm:text-xl">
                                        <FileSignature className="text-primary h-4 w-4 sm:h-5 sm:w-5" />
                                        Upload Progress
                                    </CardTitle>
                        ...
                        <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md lg:col-span-2">
                            <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                ...
                                <CardDescription className="text-sm">
                                    Upload required and optional documents for Performance Bond. Files will be permanently saved.
                                </CardDescription>
                            </CardHeader>
