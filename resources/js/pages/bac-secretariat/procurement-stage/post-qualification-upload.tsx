...
export default function PostQualificationUpload({ procurement, documentGuide, uploadedDocuments = [] }: PostQualificationUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Post Qualification - ${procurement?.id || 'Unknown ID'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        router.post(
            markStageComplete({ pr_number: procurement.id, stage: 'post_qualification' }).url,
            {},
            {
                onSuccess: () => {
