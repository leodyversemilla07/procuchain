...
export default function PreBidConferenceUpload({ procurement, documentGuide, uploadedDocuments = [] }: PreBidConferenceUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Pre-Bid Conference - ${procurement?.pr_number || 'Unknown'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        setIsMarkingComplete(true);
        router.post(
            markStageComplete({ pr_number: procurement.pr_number, stage: 'pre_bid_conference' }).url,
            {},
            {
                onSuccess: (page) => {
    ...
    const isStageCompleted = procurement.status?.includes('pre_bid_conference') === false || procurement.stage !== 'pre_bid_conference';
    ...
    const handleConfirmUpload = useCallback(() => {
        ...
        router.post(
            uploadSingleDocument({ pr_number: procurement.pr_number, stage: 'pre_bid_conference' }).url,
            {
                document_file: file,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Upload Pre-Bid Conference" />
            ...
            <div className="from-background to-muted/20 flex h-full flex-1 flex-col gap-4 rounded-xl bg-linear-to-b p-4 sm:gap-6 sm:p-6">
                <div className="flex flex-col gap-2">
                    <div className="text-primary flex items-center gap-2">
                        ...
                        <h1 className="text-xl font-bold sm:text-2xl">Pre-Bid Conference</h1>
                    </div>
                    <p className="text-muted-foreground text-sm sm:text-base">
                        Upload the Pre-Bid Conference for procurement
                        ...
                        <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md lg:col-span-2">
                            <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                ...
                                <CardDescription className="text-sm">
                                    Upload required and optional documents for Pre-Bid Conference. Files will be permanently saved.
                                </CardDescription>
                            </CardHeader>
