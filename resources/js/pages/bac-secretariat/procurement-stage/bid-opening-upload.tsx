...
import { AlertCircle, ArrowRight, CheckCircle2, FolderOpen } from 'lucide-react';
import React, { useCallback, useState } from 'react';
import { toast } from 'sonner';

interface BidOpeningUploadProps {
    procurement: {
        pr_number: string;
        title: string;
        status?: string;
...
export default function BidOpeningUpload({ procurement, workflowInfo, documentGuide, uploadedDocuments = [] }: BidOpeningUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Bid Opening - ${procurement?.pr_number || 'Unknown'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        setIsMarkingComplete(true);
        router.post(
            markStageComplete({ pr_number: procurement.pr_number, stage: 'bid_opening' }).url,
            {},
            {
                onSuccess: (page) => {
    ...
    const handleConfirmUpload = useCallback(() => {
        ...
        router.post(
            uploadSingleDocument({ pr_number: procurement.pr_number, stage: 'bid_opening' }).url,
            {
                document_file: file,
                    ...
                    <CardContent className="flex flex-col gap-2 p-4 sm:p-6">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div className="text-primary flex items-center gap-2">
                                ...
                                <h1 className="text-xl font-bold sm:text-2xl">Bid Opening</h1>
                            </div>
                            {workflowInfo && <ModeBadge workflowInfo={workflowInfo} />}
                        </div>
                        <p className="text-muted-foreground text-sm sm:text-base">
                            Upload the Bid Opening for procurement
                            ...
                            <Card className="border-sidebar-border/70 dark:border-sidebar-border h-fit shadow-md">
                                <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                    <CardTitle className="flex items-center gap-2 text-lg font-semibold sm:text-xl">
                                        <FolderOpen className="text-primary h-4 w-4 sm:h-5 sm:w-5" />
                                        Upload Progress
                                    </CardTitle>
                        ...
                        <Card className="border-sidebar-border/70 dark:border-sidebar-border shadow-md lg:col-span-2">
                            <CardHeader className="space-y-1 pb-2 sm:pb-4">
                                ...
                                <CardDescription className="text-sm">
                                    Upload required and optional documents for Bid Opening. Files will be permanently saved.
                                </CardDescription>
                            </CardHeader>
