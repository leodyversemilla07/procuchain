...
export default function PreProcurementConferenceUpload({ procurement, documentGuide, uploadedDocuments = [] }: PreProcurementConferenceUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Pre-Procurement Conference - ${procurement?.pr_number || 'Unknown'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);
    ...
    const isStageCompleted = procurement.stage !== 'pre_procurement_conference' || 
        (procurement.status?.includes('pre_procurement_conference') === true && 
         (procurement.status?.includes('completed') || procurement.status?.includes('skipped')));
