...
export default function MonitoringUpload({ procurement, documentGuide, uploadedDocuments = [] }: MonitoringUploadProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = buildBreadcrumbs(UserRole.BAC_SECRETARIAT, [
        ...
        { title: `Upload Monitoring - ${procurement?.id || 'Unknown ID'}${procurement?.title ? ': ' + procurement.title : ''}`, href: '#' },
    ]);

    const handleMarkComplete = () => {
        router.post(
            markStageComplete({ pr_number: procurement.id, stage: 'monitoring' }).url,
            {},
            {
                onSuccess: () => {
            ...
            <div className="from-background to-muted/20 flex h-full flex-1 flex-col gap-4 rounded-xl bg-linear-to-b p-4 sm:gap-6 sm:p-6">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-muted-foreground max-w-3xl text-sm sm:text-base">
                        Upload the Monitoring for procurement
