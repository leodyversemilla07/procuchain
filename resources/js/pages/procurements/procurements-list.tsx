...
const generateCSVContent = (procurements: ProcurementListItem[]): string => {
    ...
    const rows = procurements.map((proc) => getProcurementRowData(proc).map(formatCSVValue).join(','));
...
export function DataTableColumnHeader<TData, TValue>({ column, title, className }: DataTableColumnHeaderProps<TData, TValue>) {
    if (!column.getCanSort()) {
        return <div className={cn('text-foreground max-w-[150px] truncate text-xs font-semibold sm:max-w-none', className)}>{title}</div>;
    }
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button
                        variant="ghost"
                        size="sm"
                        className="text-foreground hover:bg-accent hover:text-accent-foreground data-[state=open]:bg-accent data-[state=open]:text-accent-foreground -ml-3 h-8 max-w-[150px] justify-start truncate text-xs font-semibold sm:max-w-none"
                    >
                        <span className="truncate">{title}</span>
                        {column.getIsSorted() === 'desc' ? (
                            <ArrowDownIcon className="text-primary ml-2 h-3.5 w-3.5 shrink-0" />
                        ) : column.getIsSorted() === 'asc' ? (
                            <ArrowUpIcon className="text-primary ml-2 h-3.5 w-3.5 shrink-0" />
                        ) : (
                            <ArrowUpDown className="text-muted-foreground ml-2 h-3.5 w-3.5 shrink-0" />
                        )}
                    </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="bg-popover border-border min-w-[150px] p-1.5 shadow-md">
                    <DropdownMenuItem
                        onClick={() => column.toggleSorting(false)}
                        className={cn(
                            'text-popover-foreground flex cursor-pointer items-center rounded px-2.5 py-1.5',
                            column.getIsSorted() === 'asc' ? 'bg-accent text-accent-foreground' : 'hover:bg-accent hover:text-accent-foreground',
                        )}
                    >
                    ...
                    <DropdownMenuItem
                        ...
                        className={cn(
                            'text-popover-foreground flex cursor-pointer items-center rounded px-2.5 py-1.5',
                            column.getIsSorted() === 'desc' ? 'bg-accent text-accent-foreground' : 'hover:bg-accent hover:text-accent-foreground',
                        )}
                    >
                    ...
                    <DropdownMenuItem
                        ...
                        className="text-popover-foreground hover:bg-accent hover:text-accent-foreground flex cursor-pointer items-center rounded px-2.5 py-1.5"
                    >
                        <EyeOffIcon className="text-muted-foreground mr-2 h-3.5 w-3.5" />
    ...
    return (
        <div className="w-full">
            ...
            <div className="mb-4 flex items-center justify-between">
                {selectedRowCount > 0 ? (
                    <div className="bg-muted/30 flex flex-1 flex-col items-start justify-between gap-3 rounded-lg border p-3 sm:flex-row sm:items-center">
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-foreground text-2xl font-bold">Procurement List</h1>
                                    <p className="text-muted-foreground mt-1 text-sm">View and manage procurement items across all stages</p>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4 lg:grid-cols-4">
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            ...
                            <Archive className="text-muted-foreground h-3 w-3 md:h-4 md:w-4" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-lg font-bold md:text-2xl">{procurements.length}</div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            ...
                            <Activity className="text-muted-foreground h-3 w-3 md:h-4 md:w-4" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-lg font-bold md:text-2xl">{getInProgressCount()}</div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-lg font-bold md:text-2xl">{getCompletedCount()}</div>
                        </CardContent>
                    </Card>

                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-xs font-medium md:text-sm">Documents</CardTitle>
                            <FileText className="text-muted-foreground h-3 w-3 md:h-4 md:w-4" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-lg font-bold md:text-2xl">{getTotalDocuments()}</div>
                        </CardContent>
                    </Card>
                </div>
                <div className="pb-4">
