...
function SummaryCard({
    ...
    const content = (
        ...
        >
            <CardContent className="p-3 pt-4 sm:pt-6 sm:pb-4 sm:px-6">
                <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1 space-y-0.5 sm:space-y-1">
                        <p className="truncate text-xs sm:text-sm font-medium text-muted-foreground">{title}</p>
                        <p className={cn('text-xl sm:text-2xl font-bold tabular-nums', isValid ? 'text-foreground' : 'text-amber-600 dark:text-amber-400')}>
                            {value}
                        </p>
                        {subtitle && <p className="truncate text-[10px] sm:text-xs text-muted-foreground">{subtitle}</p>}
                        {showProgress && progressValue !== undefined && (
                            <Progress value={progressValue} className="mt-1.5 sm:mt-2 h-1 sm:h-1.5" aria-label={`${title}: ${progressValue}%`} />
                        )}
                    </div>
                    <div
                        className={cn(
                    ...
                    >
                        <Icon className={cn('h-4 w-4 sm:h-5 sm:w-5', isValid ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400')} />
                    </div>
                </div>
            </CardContent>
        </Card>
...
function CompletenessTab({ result }: { result: CompletenessResult }) {
    const progressColor =
        result.completion_percentage === 100 ? 'bg-green-500' : result.completion_percentage >= 50 ? 'bg-amber-500' : 'bg-red-500';
            ...
            <CardHeader className="pb-3 sm:pb-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        {/* Inline Stats */}
                        <div className="flex items-center gap-1.5 rounded-md border bg-muted/50 px-2 py-1 sm:px-2.5 sm:py-1.5">
                            <span className="text-xs text-muted-foreground">Required:</span>
                            <span className="text-xs sm:text-sm font-semibold tabular-nums">{result.document_counts.required}</span>
                        </div>
                        <div className="flex items-center gap-1.5 rounded-md border border-green-200 bg-green-50 px-2 py-1 sm:px-2.5 sm:py-1.5 dark:border-green-800 dark:bg-green-950/50">
                            ...
                            <span className="text-xs sm:text-sm font-semibold tabular-nums text-green-600 dark:text-green-400">{result.document_counts.uploaded}</span>
                        </div>
                        {result.document_counts.missing > 0 && (
                            <div className="flex items-center gap-1.5 rounded-md border border-red-200 bg-red-50 px-2 py-1 sm:px-2.5 sm:py-1.5 dark:border-red-800 dark:bg-red-950/50">
                                ...
                                <span className="text-xs sm:text-sm font-semibold tabular-nums text-red-600 dark:text-red-400">{result.document_counts.missing}</span>
                            </div>
                        )}
                ...
                <div className="mt-3 sm:mt-4 space-y-1.5">
                    ...
                    <div className="h-2 sm:h-2.5 overflow-hidden rounded-full bg-muted">
                        <div
                    ...
                    <details className="group">
                        ...
                        <div className="mt-3 divide-y overflow-hidden rounded-lg border">
                            {result.uploaded_documents.map((doc, index) => (
                                <div key={index} className="flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-muted/50">
            ...
            <CardHeader className="pb-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-1">
                        ...
                        <CardDescription>
                            Expected PR Number: <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-foreground">{result.pr_number}</code>
                        </CardDescription>
                    </div>
                        ...
                        <div className="divide-y overflow-hidden rounded-lg border">
                            {result.pr_number_checks.map((check, index) => (
                                <div
                                    key={index}
                                    className={cn(
                                        'flex flex-col gap-2 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:px-4 sm:py-3 transition-colors',
                                        check.matches
                                ...
                                >
                                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                                        {check.matches ? (
                                            <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 shrink-0 text-green-600" />
                                        ) : (
                                            <XCircle className="h-4 w-4 sm:h-5 sm:w-5 shrink-0 text-red-600" />
                                        )}
                                        <div className="min-w-0">
                                            <p className="truncate text-xs sm:text-sm font-medium">{check.document_type_display}</p>
                                            <p className="truncate text-[10px] sm:text-xs text-muted-foreground">{check.file_key}</p>
                                        </div>
                                    </div>
                                    <div className="ml-6 sm:ml-4 shrink-0 sm:text-right">
                                        <code
                                            className={cn(
                                                'rounded px-1.5 sm:px-2 py-0.5 sm:py-1 font-mono text-[10px] sm:text-xs',
                                                check.matches
                                        ...
                                        {!check.matches && <p className="mt-0.5 sm:mt-1 text-[10px] sm:text-xs text-red-600">Expected: {check.expected_pr_number}</p>}
                                    </div>
                                </div>
                            ))}
                        </div>
                                ...
                                >
                                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                                        {check.valid ? (
                                            ...
                                            <AlertTriangle className="h-4 w-4 sm:h-5 sm:w-5 shrink-0 text-amber-600" />
                                        )}
                                        <div className="min-w-0">
                                            <p className="truncate text-xs sm:text-sm font-medium">{check.document_type_display}</p>
                                            <p className="truncate text-[10px] sm:text-xs text-muted-foreground">{check.file_key}</p>
                                    ...
                                    <Badge
                                        ...
                                        className={cn(
                                            'ml-6 sm:ml-0 w-fit shrink-0 text-[10px] sm:text-xs',
                    ...
                    <details className="group">
                        ...
                        <div className="mt-3 divide-y overflow-hidden rounded-lg border">
                            {optionalDocs.map((check, index) => (
                                <div key={index} className="flex items-center justify-between px-4 py-2.5 transition-colors hover:bg-muted/50">
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col p-3 sm:p-4 md:p-6 lg:p-8 print:p-0">
                {/* Success Banner */}
                {allValid && (
                    <Alert className="mb-6 border-green-200 bg-green-50/50 print:hidden dark:border-green-800 dark:bg-green-950/20">
                ...
                <div className="space-y-6 print:space-y-4">
                    ...
                    <HeroCard
                        ...
                        description={
                            <span className="flex flex-wrap items-center gap-x-2 gap-y-1.5 text-sm">
                                <span className="font-medium text-foreground">PR: {report.pr_number}</span>
                                ...
                                {procurementStatus && (
                                    <>
                                        ...
                                        <Badge
                                            variant="outline"
                                            className="font-normal text-[10px] sm:text-xs border-blue-200 bg-blue-50/50 text-blue-700 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-300"
                                        >
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <span className="hidden cursor-help text-muted-foreground sm:inline">
                        ...
                        actions={
                            <div className="flex flex-col items-end gap-2 sm:flex-row sm:items-center">
                                ...
                                <div className="flex gap-1 print:hidden">
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Button variant="outline" size="sm" onClick={handleRefresh} className="h-8 sm:h-9 gap-1.5 sm:gap-2 px-2 sm:px-3">
                        ...
                        <Card
                            className={cn(
                                ...
                                report.summary.critical_issues > 0
                                    ? 'border-l-red-500 border-red-200 bg-red-50/50 dark:border-red-800 dark:bg-red-950/20'
                                    : 'border-l-yellow-500 border-yellow-200 bg-yellow-50/50 dark:border-yellow-800 dark:bg-yellow-950/20',
                            )}
                        >
                    ...
                    <Tabs defaultValue="integrity" className="w-full">
                        <TabsList className="grid h-auto w-full grid-cols-4 gap-0.5 sm:gap-1 p-0.5 sm:p-1">
                            <TabsTrigger value="integrity" className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm data-[state=active]:bg-background">
                                <div className="relative">
                                    <FileCheck className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                                    {!report.summary.integrity_valid && (
                                        <span className="absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-red-500" />
                                    )}
                                </div>
                                <span className="hidden sm:inline">Integrity</span>
                            </TabsTrigger>
                            <TabsTrigger value="completeness" className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm data-[state=active]:bg-background">
                                <div className="relative">
                                    <FileText className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                                    {!report.completeness_result.is_complete && (
                                        <span className="absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-amber-500" />
                                    )}
                                </div>
                                <span className="hidden sm:inline">Completeness</span>
                            </TabsTrigger>
                            <TabsTrigger value="crossref" className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm data-[state=active]:bg-background">
                                <div className="relative">
                            ...
                            <TabsTrigger value="compliance" className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm data-[state=active]:bg-background">
                                <div className="relative">
                        ...
                        <TabsContent value="integrity" className="mt-4 focus-visible:outline-none focus-visible:ring-0">
                            <IntegrityTab results={report.integrity_results} />
                        </TabsContent>

                        <TabsContent value="completeness" className="mt-4 focus-visible:outline-none focus-visible:ring-0">
                            <CompletenessTab result={report.completeness_result} />
                        </TabsContent>

                        <TabsContent value="crossref" className="mt-4 focus-visible:outline-none focus-visible:ring-0">
                            <CrossReferenceTab result={report.cross_reference_result} />
                        </TabsContent>

                        <TabsContent value="compliance" className="mt-4 focus-visible:outline-none focus-visible:ring-0">
                    ...
                    <Card className="bg-muted/30 print:bg-transparent">
                        <CardContent className="p-3 sm:py-4 sm:px-6">
                            <div className="flex flex-col gap-1.5 sm:gap-2 text-xs sm:text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
