...
import { useUserManagement, getRoleDisplayName } from '@/hooks/use-user-management';
...
export default function AdminUserManagement() {
  const {
    ...
    selectedUser, userToDelete, formData, setFormData,
      ...
      <div className="from-background to-muted/20 flex h-full flex-1 flex-col gap-4 rounded-xl bg-linear-to-b p-4 sm:gap-6 sm:p-6">
        <HeroCard
          ...
          actions={
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button onClick={toggleAutoRefresh} variant={autoRefresh ? 'default' : 'outline'} size="sm" className="h-9 gap-1 sm:h-8 sm:gap-2">
        ...
        <StatsGrid
          items={[
            ...
            { label: `Email Verified (${stats.verifiedPercentage}%)`, value: stats.verified, icon: CheckCircle2, iconClassName: 'bg-success/10 text-success' },
            { label: `2FA Enabled (${stats.twoFactorPercentage}%)`, value: stats.twoFactor, icon: Shield, iconClassName: 'bg-blue-500/10 text-blue-600' },
        ...
        <CreateUserDialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen} formData={formData} setFormData={setFormData} roles={roles} onSubmit={handleCreateUser} getRoleDisplayName={getRoleDisplayName} />
        <EditUserDialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen} formData={formData} setFormData={setFormData} roles={roles} onSubmit={handleEditUser} getRoleDisplayName={getRoleDisplayName} />
        ...
        <BulkDeleteDialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen} selectedUsers={table.getFilteredSelectedRowModel().rows.map((row) => row.original)} onConfirm={confirmBulkDelete} />
        <UserDetailsSheet open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen} user={selectedUser ? { ...selectedUser } : null} />
        <UserLoginHistorySheet open={isLoginHistoryDialogOpen} onOpenChange={setIsLoginHistoryDialogOpen} userId={selectedUser?.id ?? null} userName={selectedUser?.name} />
