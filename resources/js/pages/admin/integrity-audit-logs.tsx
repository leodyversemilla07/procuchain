...
export default function IntegrityAuditLogs() {
 const { violationTypes, recoveryStatuses, severityLevels, sources } = usePage<PageProps>().props;
 ...
 const unresolvedTotal = logs?.total ? null : 0; // We'll show total from pagination
 ...
 <SelectTrigger className="w-48"><SelectValue placeholder="Violation Type" /></SelectTrigger>
 <SelectContent>
 ...
 <Button variant="ghost" size="sm" className="ml-auto" onClick={() => setRepairResult(null)}>Dismiss</Button>
 </CardContent>
 </Card>
 )}
 ...
 <Empty>
 <EmptyMedia><FileSearch className="h-16 w-16 text-muted-foreground" /></EmptyMedia>
 <EmptyHeader><EmptyTitle>No Audit Logs</EmptyTitle></EmptyHeader>
 ...
 return (
 <TableRow key={log.id} className={log.recovery_status === 'pending' ? 'bg-yellow-50/50 dark:bg-yellow-950/20' : ''}>
 <TableCell>
 ...
 ? <Badge variant="outline" className="font-mono text-xs">#{log.revision_number}</Badge>
 : '—'}
 </TableCell>
 <TableCell>
 {log.txid ? (
 ...
 <TooltipContent><code className="text-xs break-all">{log.txid}</code></TooltipContent>
 </Tooltip>
 </TooltipProvider>
 ) : '—'}
 </TableCell>
 ...
 <TooltipContent><code className="text-xs break-all">{log.verification_run_id}</code></TooltipContent>
 </Tooltip>
 </TooltipProvider>
 </TableCell>
 <TableCell className="text-sm text-muted-foreground">
 ...
 This will re-sync the data from the blockchain, restoring the mirror record to match the on-chain truth.
 Violation: <strong>{violationTypes[log.violation_type] ?? log.violation_type}</strong> on PR <code>{log.stream_key}</code>
 </AlertDialogDescription>
 </AlertDialogHeader>
 <AlertDialogFooter>
 ...
 <DialogDescription>
 Permanent forensic record for this violation
 </DialogDescription>
 </DialogHeader>
 {selectedLog && (
 ...
 <div>
 <span className="text-muted-foreground">Violation Type:</span><br />
 <strong>{violationTypes[selectedLog.violation_type] ?? selectedLog.violation_type}</strong>
 </div>
 <div>
 <span className="text-muted-foreground">Severity:</span><br />
 <Badge className={SEVERITY_COLORS[selectedLog.severity]}>{selectedLog.severity}</Badge>
 </div>
 <div>
 <span className="text-muted-foreground">PR Number:</span><br />
 <code>{selectedLog.stream_key}</code>
 </div>
 <div>
 <span className="text-muted-foreground">Stream:</span><br />
 {selectedLog.stream}
 </div>
 <div>
 <span className="text-muted-foreground">Recovery Status:</span><br />
 ...
 <code className="break-all text-xs">{selectedLog.txid ?? '—'}</code>
 </div>

 {/* Revision Tracking */}
 {selectedLog.revision_number !== null && selectedLog.revision_number !== undefined && (
 <div>
 <span className="text-muted-foreground">Revision:</span><br />
 ...
 <code className="text-xs text-muted-foreground" title={txid}>
 {txid.slice(0, 12)}…
 </code>
 </span>
 ))}
 ...
 <div>
 <span className="text-muted-foreground">Verification Run ID:</span><br />
 <code className="break-all text-xs">{selectedLog.verification_run_id}</code>
 </div>

 {selectedLog.field_differences && (
 <div>
 <span className="text-muted-foreground">Field Differences:</span><br />
 ...
 <div>
 <span className="text-muted-foreground">Mirror Snapshot (DB):</span><br />
 ...
 <div>
 <span className="text-muted-foreground">Chain Snapshot (Blockchain):</span><br />
 ...
 <div>
 <span className="text-muted-foreground">Recovery Result:</span><br />
 ...
 <div>
 <span className="text-muted-foreground">Recovered:</span><br />
 ...
 <Dialog open={!!reportRunId} onOpenChange={() => { setReportRunId(null); setReportData(null); }}>
 <DialogContent className="max-w-2xl">
 <DialogHeader>
 <DialogTitle className="flex items-center gap-2">
