    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="from-background to-muted/20 flex h-full flex-1 flex-col gap-4 rounded-xl bg-linear-to-b p-4 sm:gap-6 sm:p-6">
          ...
          actions={
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button onClick={() => setAutoRefresh(!autoRefresh)} variant={autoRefresh ? 'default' : 'outline'} size="sm">
                {autoRefresh ? (
                  <><Spinner className="size-4" /><span className="hidden sm:ml-2 sm:inline">Auto-refresh On</span></>
                ) : (
                  <><Clock className="h-4 w-4" /><span className="hidden sm:ml-2 sm:inline">Enable Auto-refresh</span></>
                )}
              </Button>
              <Button onClick={() => exportToCSV()} variant="outline" disabled={isExporting || combinedFilteredAndSortedLogs.length === 0} size="sm">
                {isExporting ? (
                  <><Spinner className="size-4" /><span className="hidden sm:ml-2 sm:inline">Exporting...</span></>
                ) : (
                  <><Download className="h-4 w-4" /><span className="hidden sm:ml-2 sm:inline">Export CSV</span></>
                )}
              </Button>
            </div>
          }
        ...
        <LoginLogFilterBar
          ...
          showAdvancedFilters={showAdvancedFilters} onToggleAdvanced={() => setShowAdvancedFilters(!showAdvancedFilters)}
          ...
          selectedDeviceType={selectedDeviceType} onDeviceTypeChange={setSelectedDeviceType}
          selectedBrowser={selectedBrowser} onBrowserChange={setSelectedBrowser}
          dateRange={dateRange} onDateRangeChange={setDateRange}
          onDateRangePreset={setDateRangePreset}
          uniqueRoles={getUniqueRoles} uniqueBrowsers={getUniqueBrowsers} uniqueDeviceTypes={getUniqueDeviceTypes}
            ...
            <Card>
              <CardContent className="space-y-4 p-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-1"><Skeleton className="h-6 w-48" /><Skeleton className="h-4 w-64" /></div>
                  <Skeleton className="h-9 w-24" />
                </div>
                <div className="space-y-3">
                  {Array.from({ length: 8 }).map((_, i) => (
                    <div key={`table-skeleton-${i}`} className="flex items-center gap-4 border-b pb-3">
                      <Skeleton className="h-4 w-4" /><Skeleton className="h-6 w-20" />
                      <div className="flex-1 space-y-2"><Skeleton className="h-4 w-32" /><Skeleton className="h-3 w-48" /></div>
                      <Skeleton className="h-6 w-24" /><Skeleton className="h-6 w-16" /><Skeleton className="h-4 w-28" /><Skeleton className="h-8 w-8" />
                    </div>
                  ))}
                </div>
              </CardContent>
        ...
        >
          <div className="flex-1">
            <LoginLogTable
              ...
              onPageSizeChange={(size) => { setPageSize(size); setCombinedPage(1); }}
            />
          </div>
        </Deferred>
