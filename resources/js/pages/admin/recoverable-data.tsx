...
async function pollNodeOperationStatus(
    ...
    return new Promise((resolve) => {
        const interval = setInterval(async () => {
            try {
                const res = await fetch(adminRecoverableData.nodeOperationStatus.url({ jobId }));
...
export default function RecoverableDataPage({ deletedFiles, nodes, flash }: RecoverableDataPageProps) {
    ...
    const handleFullPurgeFromNode = useCallback(async () => {
        ...
        try {
            ...
            // Poll for completion
            const result = await pollNodeOperationStatus(jobId, (status, message) => {
                ...
                <Card className="border-amber-500/20 bg-amber-500/5">
                    ...
                    <CardContent className="space-y-4">
                        <FieldDescription>
                            ...
                            is wiped — but data survives on other nodes. Use <strong className="text-foreground">manual resync</strong> to restore the
                            purged node's local copy from peers. Every purge is recorded on-chain as an audit event (RA 12009 NGPA).
                        </FieldDescription>
                        ...
                        <div className="bg-muted/50 space-y-2 rounded-md p-3 text-sm">
                            ...
                            <ol className="text-muted-foreground list-decimal space-y-1 pl-4 text-sm">
                                ...
                                <li>
                                    <strong className="text-foreground">Resync</strong> — Use the Manual Resync section below to restore the purged
                                    node's data from peers
                                </li>
                                <li>
                                    <strong className="text-foreground">Confirm</strong> — Both the purge and resync events are on-chain; data is
                                    fully restored, proving blockchain recoverability
                                </li>
                            </ol>
                        </div>
                                        ...
                                        <SelectGroup>
                                            {nodes.map((node) => (
                                                <SelectItem key={node.id} value={node.id} disabled={node.is_purged}>
                                                    {node.name} ({node.role})
                                                    {node.is_purged
                                                        ? ` — 🔴 Purged${node.last_action === 'resynced' ? ' (resynced)' : ''}`
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            ...
                                            storage. Data survives on other nodes — use Manual Resync to restore this node's local copy. Both the
                                            purge and resync events are recorded on-chain for audit compliance (RA 12009).
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                    ...
                    <Card className="border-emerald-500/20 bg-emerald-500/5">
                        ...
                        <CardContent className="space-y-4">
                            <FieldDescription>
                                ...
                                Both the purge and resync events are recorded on-chain for RA 12009 audit compliance.
                            </FieldDescription>
