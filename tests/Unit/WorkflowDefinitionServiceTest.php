...
it('prefers active workflow and document configuration rows over defaults', function () {
    ProcurementWorkflowConfig::query()->create([
        'procurement_mode' => ProcurementMode::NEGOTIATED_PROCUREMENT->value,
    ...
    $stages = $this->service->getStagesForMode(ProcurementMode::NEGOTIATED_PROCUREMENT);
    $requiredDocuments = $this->service->getRequiredDocuments(StageEnums::BAC_RESOLUTION, ProcurementMode::NEGOTIATED_PROCUREMENT);
    $optionalDocuments = $this->service->getOptionalDocuments(StageEnums::BAC_RESOLUTION, ProcurementMode::NEGOTIATED_PROCUREMENT);
    ...
    expect($this->service->isStageOptional(StageEnums::BAC_RESOLUTION, ProcurementMode::NEGOTIATED_PROCUREMENT))->toBeTrue()
...
it('falls back to mode-aware defaults when no active configuration exists', function () {
    ...
    expect($this->service->getStagesForMode(ProcurementMode::SMALL_VALUE_PROCUREMENT))->toBe($defaultStages)
...
it('builds a stage document guide with counts and metadata from the resolved definition', function () {
    ...
    expect($guide['stage'])->toBe(StageEnums::REQUEST_FOR_QUOTATION->value)
        ...
        ->and($guide['mode'])->toBe(ProcurementMode::SMALL_VALUE_PROCUREMENT->value)
