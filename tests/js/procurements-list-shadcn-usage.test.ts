...
test('procurements list and toolbar follow base-ui composition patterns', () => {
    ...
    const toolbarSource = readFileSync(path.join(process.cwd(), 'resources/js/components/procurements-list/procurement-filters-toolbar.tsx'), 'utf8');
    ...
    assert.match(toolbarSource, /className=\{cn\('flex flex-col gap-3 pb-4 sm:gap-4', className\)\}/);
    assert.match(toolbarSource, /className="flex h-9 w-full items-center justify-center gap-2 sm:w-auto"/);
