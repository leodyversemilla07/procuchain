...
test('workflow config cards use official card composition and wrap card content safely', () => {
    ...
    assert.match(source, /<CardHeader>\s*<CardTitle className="text-lg">\{config\.display_name\}<\/CardTitle>\s*<CardDescription>\{config\.irr_section\}<\/CardDescription>\s*<\/CardHeader>/s);
    ...
    assert.match(source, /className="flex-1 min-w-\[140px\]"/);
